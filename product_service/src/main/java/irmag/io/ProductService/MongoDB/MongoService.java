...
public class MongoService {
    ...
    private void ensureIndexExists() {
        ...
        existingTextIndexes.stream()
                .filter(index -> !requiredIndexes.contains(index))
                .forEach(index -> dropIndex(collectionName, index));

        requiredIndexes.stream()
                .filter(field -> !existingTextIndexes.contains(field))
                .forEach(field -> mongoTemplate.getCollection(collectionName)
                        .createIndex(Indexes.text(field)));
    }
    ...
    private List<String> getTextIndexes(String collectionName) {
        return mongoTemplate.getCollection(collectionName)
                ...
                .into(new ArrayList<>())
                .stream()
                .filter(index -> index.get("key") instanceof Document
                        && ((Document) index.get("key")).containsValue("text"))
